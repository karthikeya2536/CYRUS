-- Update graph_render_relations function with improved filtering, scoring, and cycle detection
...
BEGIN
    ...
    WITH RECURSIVE traverse AS (
        ...
        SELECT
            ...
            0 AS hops,
            ARRAY[id] AS path_ids,  -- track visited nodes in this path
        ...
        SELECT
            ...
            e.base_score *
                CASE
                    ...
                    WHEN e.relationship_type IN ('associated_with', 'regarding', 'involved_in', 'mentioned', 'interested_in') THEN 0.1
                    ELSE 1.0
                END *
                CASE
                    -- Apply boost for matching intent
                    WHEN p_graph_intent = 'blocking' AND e.relationship_type IN ('blocked_by', 'requires', 'depends_on') THEN 2.0
                    WHEN p_graph_intent = 'who' AND e.relationship_type IN ('collaborates_on', 'contact_for', 'mentioned_by') THEN 2.0
                    WHEN p_graph_intent = 'working_on' AND e.relationship_type IN ('works_on', 'collaborates_on') THEN 2.0
                    ELSE 1.0
                END AS score
        FROM edges e
    )
