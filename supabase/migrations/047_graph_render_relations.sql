...
BEGIN
    ...
    WITH RECURSIVE traverse AS (
        ...
        WHERE e.user_id = p_user_id
          ...
          AND NOT (e.target_node_id = t.path_source_id) -- Avoid immediate cycle
    ...
    boosted AS (
        SELECT
            ...
            score,
            CASE
                WHEN p_graph_intent = 'blocking' AND relationship_type IN ('blocked_by', 'requires', 'depends_on') THEN score * 2.0
                WHEN p_graph_intent = 'who' AND relationship_type IN ('collaborates_on', 'contact_for', 'mentioned_by') THEN score * 2.0
                WHEN p_graph_intent = 'working_on' AND relationship_type IN ('works_on', 'collaborates_on') THEN score * 2.0
                ELSE score
            END AS boosted_score
        FROM edges
    )
    SELECT source_node, relationship_type, target_node, boosted_score as score
    FROM boosted
