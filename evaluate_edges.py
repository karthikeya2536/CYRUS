...
def run_sql(sql):
    ...
    try:
        ...
        print(f"Exception parsing JSON: {e}", file=sys.stderr)
        print("Output was:", result.stdout[:200], file=sys.stderr)
        return None
    ...
    for r in rows:
        ...
        for i, (rt, tn) in enumerate(retrieved[:10], start=1):
            ...
            if i == 1:
                ...
                dcg += rel / math.log2(i)
        # Ideal DCG: we have exactly one relevant item (the exact match). If there could be multiple edges to same target with
        # different types? but assume at most one.
