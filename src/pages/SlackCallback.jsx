...
export default function SlackCallback() {
  ...
  useEffect(() => {
    async function handleCallback() {
      ...
      if (errorParam) {
        setError(`Slack OAuth error: ${errorParam}`);
        return;
      }
      if (!code) {
      ...
      if (!state) {
        setError('Missing OAuth state from Slack.');
        return;
      }
      ...
      try {
        ...
        const { data, error: fnError } = await supabase.functions.invoke('slack-oauth-exchange', {
    ...
    <div className="auth-screen">
      <div className="auth-card" style={{ textAlign: 'center' }}>
        ...
        <p className="muted" style={{ marginTop: 'var(--space-4)' }}>
          <span className="spinner" aria-hidden="true" /> Connecting your Slack workspace…
        </p>
      </div>
    </div>
  );
